...
| 항목 | 우선순위 | 관련 기능 |
|------|----------|----------|
| User 도메인 (경고/차단) | 높음 | 노쇼 처리 |
| Outbox 패턴 | 중간 | 이벤트 신뢰성 |
| 즉시 구매 로직 | 높음 | 입찰 |
| 경매 취소 기능 | 중간 | 경매 관리 |
| Transaction 도메인 | 중간 | 거래 관리 |
...
1. **User 도메인 구현**
   - 경고 횟수 관리
   - 차단 로직 구현
   - NoShowProcessingService의 TODO 해결

2. **즉시 구매 기능 구현**
   - 즉시 구매 입찰 타입 추가
   - 1시간 최종 입찰 기회 로직
   - 90% 이상 시 비활성화 로직 (이미 완료)
