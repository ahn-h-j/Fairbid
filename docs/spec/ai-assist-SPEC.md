...
2026-04-20 스모크 게이트 판정 (Gemini 설명 쌍비교 승률 58.1% ≥ 40%) 에 근거해 **옵션 B 를 이슈 #91 / PR #92 로 구현 완료**. 가격=Claude Sonnet 4.5 / 설명=Gemini 2.5 Pro 의 역할 분할 구조가 프로덕션 기본이 되었다 (phase2a/phase2b 병렬 호출). 옵션 A(단순 프로바이더 교체) 는 미채택. 로컬 모델(Gemma 3, Qwen2.5-VL)은 GPU 인프라 부담 때문에 장기 옵션으로 보류.
