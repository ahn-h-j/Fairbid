 ...
 * 스파이크 테스트
 *
 * 시나리오: 갑작스러운 트래픽 급증 시뮬레이션
 * 목적: 순간적인 부하 폭증에 대한 시스템 복원력 측정
 *
 * 실행: k6 run k6/scenarios/bid-spike.js
 */
...
export const options = {
    ...
    scenarios: {
        // 스파이크: 급격한 트래픽 증가 후 감소 (총 90초)
        spike: {
            executor: 'ramping-vus',
            startVUs: 0,
            stages: [
                { duration: '10s', target: 10 },   // 워밍업
                { duration: '5s', target: 200 },   // 급격한 스파이크!
                { duration: '30s', target: 200 },  // 피크 유지
                { duration: '5s', target: 10 },    // 급격한 감소
                { duration: '30s', target: 10 },   // 안정화 관찰
    ...
    thresholds: {
        http_req_duration: ['p(95)<1000', 'p(99)<2000'], // 스파이크 시 더 관대한 기준
        http_req_failed: ['rate<0.1'],  // 10%까지 허용
        bid_error_rate: ['rate<0.2'],   // 스파이크 시 20%까지 허용
    },
};
...
export function setup() {
    console.log('🚀 스파이크 테스트 셋업 시작...');

    const sellerId = 9999;
    const headers = getHeaders(sellerId);

    const auctionPayload = JSON.stringify({
        title: `스파이크테스트 경매 ${Date.now()}`,
        description: '스파이크 테스트용 경매입니다.',
...
export default function (data) {
    ...
    sleep(Math.random() * 0.5 + 0.2); // 0.2~0.7초 (스파이크 시 더 빠른 요청)
}
...
export function handleSummary(data) {
    return {
        ...
        'k6/results/bid-spike-result.json': JSON.stringify(data, null, 2),
    };
}
